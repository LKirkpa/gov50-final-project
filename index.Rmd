...
ggplot(edu_diff) + 
         geom_col(aes(x = race, y = income_mean, fill = race)) +
         facet_wrap(~edu, labeller = label_wrap_gen(width=0.5)) + theme(strip.background = element_rect(fill="beige", size= 0.5, color="brown"), panel.spacing.x = unit(1, "cm")) + labs(title = "Race & Income on Education", x = "Race of Respondent", y = "Avg. Income", caption = "American Community Survey 2000 (U.S. Census Bureau)")
...
```{r}
ate <- acs |>
  mutate(Race_Respondent = if_else(race == "White", "White", "BIPOC"), 
         College_Exposure = if_else(edu < 3, "Less Than College", "College Exposed")) |>
  group_by(Race_Respondent, College_Exposure) |>
  summarize(income_mean = mean(income)) |>
  pivot_wider(names_from = Race_Respondent,
                values_from = income_mean) |>
  mutate(ATE = White - BIPOC)
...
ate_race <- acs |>
  ...
  summarize(edu_mean = mean(edu)) |>
  pivot_wider(names_from = c(race, income), 
              values_from = mean(edu)) |>
  mutate(ATE_Race = 
           ATE_Income = 
           ATE_Diff = ATE_Race - ATE_Income)
