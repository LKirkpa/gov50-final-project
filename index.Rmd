...
ggplot(data = WHDF2023, aes(x = GDP_per_capita, y = happiness_score)) + geom_point(size = 1.5) +
  ...
  geom_vline(xintercept = c(6.75,7.25) + geom_point(aes(x = mean(GDP_per_capita) , y = mean_happ_7), color = "lightgoldenrod", size = 4)
             

```
```{r}
ggplot(data = WHDF2019_10, aes(x = GDP_per_capita, y = happiness_score, color = country)) + geom_point(size = 1.5) + 
...
```{r}
fit <- lm(happiness_score ~ 1 + GDP_per_capita + social_support + generosity, data = WHDF2019_VS_WHDF2023)
